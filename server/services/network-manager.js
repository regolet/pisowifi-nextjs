...
const { isValidMacAddress, sanitizeMacAddress, isValidDuration, isValidIPv4 } = require('../utils/validators');
...
`;
  ...
  async authenticateClient(macAddress, ipAddress, duration) {
    try {
      // SECURITY: Validate all inputs before shell execution
      if (!isValidMacAddress(macAddress)) {
        console.error('Invalid MAC address format:', macAddress?.substring(0, 20));
      ...
      if (!isValidDuration(duration)) {
        ...
        return { success: false, error: 'Invalid duration (must be 1-86400 seconds)' };
      }

      const sanitizedMac = sanitizeMacAddress(macAddress);
      const sanitizedDuration = parseInt(duration, 10);

      // Use the dedicated ethernet allow script
      const scriptsPath = path.join(__dirname, '../../scripts');
      await execAsync(`sudo ${scriptsPath}/pisowifi-allow-client-ethernet ${sanitizedMac} ${sanitizedDuration}`);
  ...
  async deauthenticateClient(macAddress) {
    try {
      ...
      const scriptsPath = path.join(__dirname, '../../scripts');
      await execAsync(`sudo ${scriptsPath}/pisowifi-block-client-ethernet ${sanitizedMac} "api_disconnect"`);
  ...
  async getConnectedClients() {
    try {
      ...
      try {
        const { stdout: arpOutput } = await execAsync('ip neighbor show dev enx00e04c68276e | grep 10.0.0');
